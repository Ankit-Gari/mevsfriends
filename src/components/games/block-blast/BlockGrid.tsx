import {
	...
	HITBOX_SIZE,
	PossibleBoardSpots,
} from "@/constants/games/block-blast/Board";
import { colorToHex } from "@/constants/games/block-blast/Color";
import { Hand } from "@/constants/games/block-blast/Hand";
import {
	createEmptyBlockStyle,
	createFilledBlockStyle,
} from "@/constants/games/block-blast/Piece";
...
// One real component per cell — NOT a plain function called in a loop. The
// previous version called hooks (useSharedValue/useAnimatedReaction/
// useEffect/useAnimatedStyle) from inside a function invoked 64x per render
// from within BlockGrid's own body, which violates the rules of hooks (React
// requires a fixed hook sequence per *component* instance, not per loop
// iteration of a helper function). It happened to work most of the time but
// crashed with "Should have a queue. You are likely calling Hooks
// conditionally" on some real navigations — confirmed via a fresh, non-HMR
// browser session, not just an HMR artifact as originally suspected.
...
function BlockCell({ x, y, board, possibleBoardDropSpots }: BlockCellProps) {
	...
	return (
		<Animated.View style={[styles.emptyBlock, blockPositionStyle, animatedStyle]}>
			<BlockDroppable x={x} y={y} style={styles.hitbox} possibleBoardDropSpots={possibleBoardDropSpots}></BlockDroppable>
		</Animated.View>
	);
}
...
export default function BlockGrid({
	...
	forEachBoardBlock(board.value, (_block, x, y) => {
		blockElements.push(
			<BlockCell key={`av${x},${y}`} x={x} y={y} board={board} possibleBoardDropSpots={possibleBoardDropSpots} />
		);
	});
...
const styles = StyleSheet.create({
	...
	grid: {
		...
		position: "relative",
		// Deeper/flatter than the page's jewel-tone gradient so the board reads
		// as its own panel rather than blending into the background.
		backgroundColor: "rgba(10, 6, 24, 0.75)",
		borderWidth: 3,
		borderRadius: 5,
		borderColor: "rgba(196, 174, 255, 0.85)",
		opacity: 1,
	},
	hitbox: {
		width: HITBOX_SIZE,
