import { Color, colorToHex } from "@/constants/games/block-blast/Color";
import { mulberry32 } from "@/constants/games/block-blast/Rng";
...
// Ranked mode ONLY: piece shape comes from the seeded generator so every
// player sees the identical sequence; color stays Math.random() (do-not-seed list).
export function getSeededPiece(rngState: number): { piece: PieceData; nextState: number } {
	"worklet";
	const { value, nextState } = mulberry32(rngState);
	let position = value * totalDistributionPoints;
	let piece: PieceDataSaved;
	for (let i = 0; i < piecesData.length; i++) {
		position -= piecesData[i].distributionPoints;
		if (position < 0) {
	...
	return { piece: { ...piece!, color: getRandomPieceColorWorklet() }, nextState };
}
